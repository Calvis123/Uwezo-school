...
export function StudentPromotion() {
  ...
  const handleApprove = async (promotionId: string) => {
    ...
    try {
      const res = await promotionsApi.approve(promotionId)
      if (res.success) {
        toast.success('Promotion approved')
        loadPromotions()
      } else {
        toast.error(res.error || 'Failed to approve promotion')
      }
    } catch {
  ...
  const handleComplete = async (promotionId: string) => {
    setActionLoadingId(promotionId)
    try {
      const res = await promotionsApi.complete(promotionId)
      if (res.success) {
        toast.success('Promotion completed')
        loadPromotions()
      } else {
        toast.error(res.error || 'Failed to complete promotion')
      }
    } catch {
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/80">
                    ...
                    {canManagePromotions && <th className="text-left p-3 font-medium text-slate-500 dark:text-slate-400">Actions</th>}
                  </tr>
                </thead>
                <tbody>
                  {promotions.map((p, i) => {
