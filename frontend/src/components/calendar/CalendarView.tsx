      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
            {isBursar ? 'Payment Calendar' : 'School Calendar'}
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {isBursar
              ? `${events.length} fee-related reminders/events in ${MONTHS[currentMonth]} ${currentYear}`
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">{isBursar ? 'Add Fee Reminder' : 'Add Event'}</span>
          </Button>
        </div>
      </div>
      ...
      {isBursar && (
        <div className="rounded-xl border border-emerald-200/70 dark:border-emerald-800/40 bg-emerald-50/50 dark:bg-emerald-900/10 px-4 py-3">
          <p className="text-sm font-medium text-emerald-800 dark:text-emerald-300">
            Bursar tip: track fee deadlines, bank reconciliation dates, and parent billing reminders here.
          </p>
        </div>
      )}
