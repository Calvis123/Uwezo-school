...
export async function POST(request: NextRequest) {
  try {
    ...
    const { studentIds, fromClassId, toClassId, academicYear, term, promotedBy, notes } = body

    if (!studentIds?.length || !fromClassId || !toClassId || !academicYear || !term || !promotedBy) {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to process promotion' },
      { status: 500 }
    )
  }
}
