...
import { messagesApi, refApi } from '@/lib/api'
...
export function MessagingPage() {
  ...
  const canBulkSms = ['SUPER_ADMIN', 'ADMIN', 'HEADTEACHER', 'DOS', 'SECRETARY', 'BURSAR'].includes(user?.role || '')
  ...
  const [bulkAudience, setBulkAudience] = useState<'ALL_USERS' | 'ALL_PARENTS' | 'PARENTS_BY_CLASS'>('ALL_USERS')
  const [bulkClassId, setBulkClassId] = useState('ALL_CLASSES')
  const [bulkContacts, setBulkContacts] = useState<UserOption[]>([])
  const [bulkClasses, setBulkClasses] = useState<ClassOption[]>([])
  ...
  useEffect(() => {
    if (isParent) setActiveTab('inbox')
  }, [isParent])

  useEffect(() => {
    if (!canBulkSms) return
    refApi.classes().then((res) => {
      if (res.success && res.data) {
        setBulkClasses(Array.isArray(res.data) ? res.data : [])
      }
    }).catch(() => {})
  }, [canBulkSms])
  ...
  const loadMessages = async () => {
    ...
    try {
      const [inboxRes, sentRes] = await Promise.all([
        ...
        isParent ? Promise.resolve({ success: true, data: { messages: [] } } as any) : messagesApi.list(user.id, 'sent'),
      ])
      if (inboxRes.success && inboxRes.data) {
        setMessages(inboxRes.data.messages)
        setUnreadCount(inboxRes.data.unreadCount)
      }
      if (!isParent && sentRes.success && sentRes.data) {
  ...
  const loadUsers = async () => {
    ...
    try {
      const res = await messagesApi.contacts({ audience: 'ALL_USERS' })
      if (res.success && res.data) {
  ...
  const loadBulkContacts = async (audience: 'ALL_USERS' | 'ALL_PARENTS' | 'PARENTS_BY_CLASS', classId?: string) => {
    setBulkLoading(true)
    try {
      const res = await messagesApi.contacts({
        audience,
        ...(audience === 'PARENTS_BY_CLASS' && classId ? { classId } : {}),
      })
      if (res.success && res.data) {
        const items = Array.isArray(res.data) ? res.data : []
        setBulkContacts(items)
      } else {
        setBulkContacts([])
      }
    } catch {
  ...
  useEffect(() => {
    if (!bulkSmsOpen || !canBulkSms) return
    if (bulkAudience === 'PARENTS_BY_CLASS' && bulkClassId === 'ALL_CLASSES') {
      setBulkContacts([])
      return
    }
    loadBulkContacts(
      bulkAudience,
      bulkAudience === 'PARENTS_BY_CLASS' ? bulkClassId : undefined
    )
    setBulkSmsRecipients([])
  }, [bulkSmsOpen, bulkAudience, bulkClassId, canBulkSms])
  ...
  const handleSend = async () => {
    if (isParent) {
      toast.error('Parents can only view messages')
      return
    }
  ...
  const smsContacts = bulkContacts.filter((u) => !!u.phone)
  const filteredSmsContacts = bulkSmsSearch.trim()
    ? smsContacts.filter((u) => {
        const q = bulkSmsSearch.toLowerCase()
        return (
  ...
  const toggleBulkRecipient = (id: string, checked: boolean) => {
  ...
  const handleSelectAllSmsRecipients = () => {
    const allIds = filteredSmsContacts.map((u) => u.id)
    setBulkSmsRecipients(allIds)
  }

  const handleClearAllSmsRecipients = () => {
    setBulkSmsRecipients([])
  }

  const handleSendBulkSms = async () => {
    if (bulkSmsRecipients.length === 0 || !bulkSmsMessage.trim()) {
      toast.error('Select recipients and enter SMS message')
      return
    }

    setBulkSmsSending(true)
    try {
      const res = await messagesApi.bulkSms({
        recipientIds: bulkSmsRecipients,
        message: bulkSmsMessage.trim(),
      })
      if (res.success && res.data) {
        toast.success(`Bulk SMS queued: ${res.data.sent} sent, ${res.data.skipped} skipped`)
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await messagesApi.delete(messageToDelete.id)
      if (!res.success) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">
              {isBursar ? 'Finance Messages' : 'Messages'}
            </h2>
        ...
        <div className="flex items-center gap-2">
            ...
            <Dialog open={composeOpen} onOpenChange={setComposeOpen}>
              <DialogTrigger asChild>
                <Button size="sm" className="gap-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg shadow-md shadow-teal-500/20">
                  ...
                  <span className="hidden sm:inline">{isBursar ? 'Compose Billing Msg' : 'Compose'}</span>
                  <PenSquare className="w-3.5 h-3.5 sm:hidden" />
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-2xl rounded-2xl border-slate-200/60 dark:border-slate-700/60 p-0 overflow-hidden">
                <DialogHeader>
                  <div className="px-6 pt-6 pb-4 border-b border-slate-100 dark:border-slate-700/60 bg-gradient-to-r from-white to-slate-50/70 dark:from-slate-900 dark:to-slate-800/60">
                  <DialogTitle className="flex items-center gap-2">
                    <div className="h-9 w-9 rounded-xl bg-teal-50 dark:bg-teal-900/40 flex items-center justify-center">
                      <Mail className="w-4 h-4 text-teal-600 dark:text-teal-400" />
                    </div>
                    {isBursar ? 'New Finance Message' : 'New Message'}
                  </DialogTitle>
                  <DialogDescription className="mt-2">
                    {isBursar
                      ? 'Send fee reminders, payment confirmations, and reconciliation follow-ups.'
                      : 'Send a message to a staff member, teacher, or parent.'}
                  </DialogDescription>
                  </div>
                </DialogHeader>
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <p className="text-[11px] text-slate-500">{users.length} available recipients</p>
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-medium flex items-center gap-1.5">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    className="min-h-[150px] resize-none bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl"
                  />
                  <div className="flex items-center justify-between text-[11px] text-slate-500">
                    <span>Keep the message clear and concise.</span>
          ...
          {canBulkSms && (
            <Dialog open={bulkSmsOpen} onOpenChange={setBulkSmsOpen}>
              <DialogTrigger asChild>
                <Button size="sm" variant="outline" className="gap-2 rounded-lg border-emerald-300 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-300 dark:hover:bg-emerald-900/20">
                  <Smartphone className="w-4 h-4" />
                  <span className="hidden sm:inline">Bulk SMS</span>
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-2xl rounded-2xl border-slate-200/60 dark:border-slate-700/60">
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2">
                  ...
                  <DialogDescription>
                    Send one SMS message to multiple recipients at once.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 mt-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={bulkAudience}
                        onValueChange={(v) => {
                          setBulkAudience(v as 'ALL_USERS' | 'ALL_PARENTS' | 'PARENTS_BY_CLASS')
                          setBulkClassId('ALL_CLASSES')
                        }}
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="PARENTS_BY_CLASS">Parents by Class</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    {bulkAudience === 'PARENTS_BY_CLASS' && (
                      <div className="space-y-2">
                        ...
                        <Select value={bulkClassId} onValueChange={setBulkClassId}>
                          ...
                          <SelectContent>
                            <SelectItem value="ALL_CLASSES">Select class...</SelectItem>
                            {bulkClasses.map((c) => (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <Label className="text-sm font-medium">Recipients ({bulkSmsRecipients.length} selected)</Label>
                      <div className="flex items-center gap-2">
                        <Button type="button" size="sm" variant="ghost" onClick={handleSelectAllSmsRecipients}>
                          Select Visible
                        </Button>
                        <Button type="button" size="sm" variant="ghost" onClick={handleClearAllSmsRecipients}>
                          Clear
                        </Button>
                      </div>
                    </div>
                    <Input
                      ...
                      className="bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl"
                    />
                    <div className="rounded-xl border border-slate-200 dark:border-slate-700 max-h-56 overflow-y-auto">
                      {bulkLoading ? (
                        <div className="px-3 py-6 text-center text-xs text-slate-500">Loading contacts...</div>
                      ) : filteredSmsContacts.length === 0 ? (
                        <div className="px-3 py-6 text-center text-xs text-slate-500">No contacts with phone numbers found.</div>
                      ) : (
                        <div className="divide-y divide-slate-100 dark:divide-slate-700">
                          {filteredSmsContacts.map((u) => (
                            <label key={u.id} className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50">
                              <Checkbox
                                checked={bulkSmsRecipients.includes(u.id)}
                                onCheckedChange={(checked) => toggleBulkRecipient(u.id, !!checked)}
                              />
                              <div className="min-w-0">
                                <p className="text-sm font-medium text-slate-900 dark:text-slate-100 truncate">{u.name}</p>
                                <p className="text-[11px] text-slate-500 truncate">{u.phone || u.email}</p>
                              </div>
                              <Badge variant="secondary" className={cn('ml-auto text-[9px]', getRoleBadgeColor(u.role))}>
                                {u.role.replace('_', ' ')}
                              </Badge>
                            </label>
                          ))}
                  ...
                  <Separator />
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" onClick={() => setBulkSmsOpen(false)} className="rounded-lg">
                      Cancel
                    </Button>
                    <Button
                      className="gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg"
                      onClick={handleSendBulkSms}
                      disabled={bulkSmsSending || bulkSmsRecipients.length === 0 || !bulkSmsMessage.trim()}
                    >
                      {bulkSmsSending ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                      Send Bulk SMS
                    </Button>
                  </div>
                </div>
      ...
      {isBursar && (
        <div className="rounded-xl border border-emerald-200/70 dark:border-emerald-800/40 bg-emerald-50/50 dark:bg-emerald-900/10 px-4 py-3">
          <p className="text-sm font-medium text-emerald-800 dark:text-emerald-300">
            Use this channel for fee reminders, receipt confirmations, and parent payment follow-ups.
          </p>
        </div>
      )}
          ...
          <CardContent className="p-0">
            {filteredMessages.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 px-4">
                ...
                {!isParent && !searchQuery && activeTab === 'inbox' && (
                  <Button
                    variant="outline"
                    size="sm"
